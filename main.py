...
while cv2.getWindowProperty("Z project", cv2.WND_PROP_VISIBLE) > 0:
    ...
    if key == 13:  # If 'enter' is pressed calculate I(point) - I(BG)
        ...
        tau, I0, y0, param_covariance_matrix = processing.draw_results_and_param(IminusBG_arr, I_point_arr, I_BG_arr)
        print("tau, I0, y0", tau, I0, y0)
        print("param_covariance_matrix", param_covariance_matrix)
        print( np.hstack(param_covariance_matrix))
        ...
        with open("error.txt", "ab") as f:
            np.savetxt(f, np.vstack(param_covariance_matrix))
